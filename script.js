...
var calcItUp = function() {
    for (var i = 0; i < bonusList.length; i++) {
        ...
        $("#newTable > tbody:last-child").append("<td><span id='name'>");
            $('#name').replaceWith(bonusList[i].name);
        $("#newTable > tbody:last-child").append("</span></td>");
        $("#newTable > tbody:last-child").append("<td><span id='percent'>");
            $('#percent').replaceWith(bonusList[i].bonusPercentage);
        $("#newTable > tbody:last-child").append("</span></td>");
        $("#newTable > tbody:last-child").append("<td><span id='bonus'>");
            $('#bonus').replaceWith(bonusList[i].totalBonus);
        $("#newTable > tbody:last-child").append("</span></td>");
        $("#newTable > tbody:last-child").append("<td><span id='comp'>");
